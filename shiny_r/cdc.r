...
rsconnect::setAccountInfo(name = 'stp9u6-john-duran',
                          token = 'E8B170E2A3DE5D916BA45379FD1BE333',
                          secret = 'Oaq7RMhvwI/HbO0oviqTVA0h1YdN6Y/MtMD1TQav',
                          server = 'posit.cloud')
library(rsconnect)
rsconnect::deployApp(appDir = /cloud/project/cdc.r, appName = "<NAME>", appTitle = "my-app")
...
ui <- fluidPage(
  titlePanel("Depression Drinking Age-adjusted Prevalence in HI by County"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  output$barPlot <- renderPlot({
    ...
    ggplot() +
      ...
      labs(title = 'Depression Drinking Age-adjusted Prevalence',
